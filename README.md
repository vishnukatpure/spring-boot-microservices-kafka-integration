...
| **API Rate Limiting **              | Designed and implemented Bucket4j-based rate limiting with configurable in-memory and Redis-backed storage, applying limits based on client IP/remote address, API endpoint, and HTTP method. Added per-API configuration and standardized HTTP 429 responses for exceeded limits.|
